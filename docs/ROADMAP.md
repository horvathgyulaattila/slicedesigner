...
>
> **Megjegyzés (2026-08-24, folytatás 52):** A projektgazdával lezajlott tervezési egyeztetés nyomán megnyílt a Phase 11 (Procedurális Height Field receptek) — a `docs/BACKLOG.md` megmaradt 1. tétele alapján. A projektgazda döntése szerint az új generátorok a meglévő `relief_generator` pluginon belül, a Wave Generatorral egyenrangú, alternatív generátor-típusokként valósulnak meg — nem önálló `MeshSource` plugin(ok)ként; ez összhangban van az `IMPLEMENTATION_PLAN.md` §22-ben már előre jelzett, jövőbeli "Generator: [Wave ▼]" választó vízióval. A pontos mechanizmus (feltehetően egy plugin-legfelső szintű `generator_type` enum `ParameterSpec`, a meglévő `WaveFunction`/`envelope_type` minta szerint) a 11.1 saját tervezési körének feladata. Az alfázisok sorrendje — a projektgazda által korábban felvázolt nehézségi sorrendet követve — 11.1 Voronoi-felszín (a 10.4-es `VoronoiNoiseField`-ből, legkisebb lépés), 11.2 Holdkráter, 11.3 Dűne, 11.4 Faerezet. Közös "Height Field Recipe" absztrakció előre NEM kerül bevezetésre — a `HeightField` generátor-független szerződése (`WAVE_FUNCTION_MODEL.md` 18–20. szakasz) önmagában elegendő keretnek tekintendő, összhangban a `RELIEF_GENERATOR_DOMAIN.md` 21. szakaszának elvével; a kérdés a 11.1 tapasztalata alapján újranyitható. A `docs/BACKLOG.md` megmaradt tétele emiatt törlésre került a Backlogból. A Phase 11 ezért 🟡 In Progress állapotban megnyílt, első aktív tételként a 11.1 (Voronoi-felszín) tervezésével.
>
> **Megjegyzés (2026-08-24, folytatás 53):** A 11.1 (Voronoi-felszín) tervezése közben előkerült, hogy a generikus `ParameterSpec`/GUI-form jelenlegi, statikus csoportosítási mechanizmusa (ADR-0017 kiegészítés, ROADMAP Phase 10.1) NEM támogat feltételes (egy másik mező értékétől függő) mezőláthatóságot — ezt az ADR-0017 kiegészítése annak idején explicit módon hatókörön kívülinek nyilvánította, a `folytatás 41` bejegyzés pedig dokumentált, elfogadott korlátként rögzítette ("ha ez a gyakorlatban zavaróvá válik, egy jövőbeli backlog-tétel tárgya lehet"). A projektgazda megerősítette, hogy ez jelenleg a program legzavaróbb tulajdonsága (az `envelope_type`/`distortion_type` minden almezője mindig látható, típustól függetlenül), és hogy ezt nem csak a leendő `generator_type`-nál, hanem visszamenőleg mindenhol meg kell oldani. A projektgazdával lezajlott egyeztetés nyomán a Phase 11 elé egy új, 11.0 tétel került beékelve — a feltételes mezőláthatóság bevezetése egy új, opcionális `ParameterSpec.visible_when: tuple[str, str] | None` mezővel (vezérlő mező neve + elvárt érték; `None` alapértelmezés = mindig látható, teljes backward compatibility), a `_GeneratorParameterForm` megfelelő GUI-bekötésével, és a meglévő `envelope_*`/`distortion_*` mezők visszamenőleges bekötésével. Ez a leendő `generator_type` (11.1) számára is elsőre helyesen, feltételes láthatósággal épül majd fel. A Phase 11 Kilépési feltétele ennek megfelelően öt tételre (11.0–11.4) bővült.
>
> **Megjegyzés (2026-08-24, folytatás 54):** A 11.0 tétel (feltételes mezőláthatóság) elkészült, review-n átment — a Szoftverarchitekt programozott diffel (bash_tool + Python `difflib`) ellenőrizte mind a négy érintett fájlt (`mesh_source_registry.py`, `parameter_panel.py`, `0017-plugin-discovery-and-parameter-schema.md`, `registration.py`) a jóváhagyott prompt ellenében — byte-pontos egyezés. Az ADR-0017 az új mechanizmussal kiegészült (`visible_when`, l. az ADR saját "Kiegészítés (2026-08-24)" szakasza). A hátralévő munka a 11.1 (Voronoi-felszín) — ez lesz az első tétel, ami ténylegesen kihasználja az új mezőláthatósági mechanizmust a leendő `generator_type` bevezetésekor.
>
> **Megjegyzés (2026-08-25, folytatás 55):** A 11.1 tétel (Voronoi-felszín) elkészült, review-n átment, és a projektgazda élőben is megerősítette — a Szoftverarchitekt programozott diffel ellenőrizte mind a hét érintett forrásfájlt és mind az öt tesztfájlt a jóváhagyott prompt ellenében, byte-pontos egyezéssel (egyetlen, saját hibaként azonosított, apró kozmetikai eltérés: a `test_voronoi_parameters.py`-ból hiányzik a testvér-fájlokban meglévő `sys.path` PEP-420-namespace boilerplate — ez a promptban maradt ki, nem implementációs hiba, funkcionálisan nem okozott regressziót). Élő tesztelés közben a projektgazdának megfogalmazódott egy jövőbeli, jelen fázison kívül eső fejlesztési irány: nem téglalap alaprajzú (pl. kör, ellipszis, hullámkontúrt követő) alaptestek, illetve áttört/lyukacsos relief-testek — mindkettő architekturálisan egy, a `HeightField`-től független `(x,y) → van-e anyag` footprint/maszk-mechanizmust igényelne, amit a jelenlegi `ReliefGeometry` nem támogat. Ez `BACKLOG.md` új 1. tételeként rögzítve, explicit módon nem aktuális igényként. A Phase 11 aktív tétele mostantól a 11.2 (Holdkráter-felszín).
>
> **Megjegyzés (2026-08-25, folytatás 56):** A 11.2 tétel (Holdkráter-felszín) elkészült, review-n átment, és a projektgazda élőben is megerősítette a végeredményt. A fejlesztés három egymást követő élő-tesztelési korrekciót igényelt (l. a 11.2 kész-jelölésének részletei a Feladata-listában): a `radius`-alapú kör-levágás (a Voronoi-cellahatárig érő, sokszögletes első tervezet javítása), az `octaves`/`lacunarity` rétegzés `min`-kombinálással (méret-változatosság, egymásba ágyazódó kráterek), és a `lacunarity` újrahasznosítása mélység-skálázásra is (méret-arányos kráter-mélység, új paraméter nélkül). Élő tesztelés közben a projektgazdának megfogalmazódott egy jövőbeli fejlesztési irány (nem téglalap alaprajzú / áttört relief-testek), amit — saját kérésére — nem dokumentáltunk újra, mivel ez már szerepel a `BACKLOG.md` 1. tételében. A Phase 11 aktív tétele mostantól a 11.3 (Dűne-felszín).
...
* ~~11.0 — Feltételes (egy másik mező értékétől függő) mezőláthatóság a generikus `ParameterSpec`/GUI-formban, az `envelope_type`/`distortion_type` és a leendő `generator_type` almezőire — ADR-0017 kiegészítés~~ — kész: `ParameterSpec` új, hetedik `visible_when: tuple[str, str] | None = None` mezővel bővült; a `_GeneratorParameterForm` rekurzív `_effective_visible()`/`_update_conditional_visibility()` metódusokkal kaszkádoló (láncolt) feltételes mezőláthatóságot valósít meg, a vezérlő enum-widgetek `currentIndexChanged` jelzésére. Visszamenőlegesen bekötve mindhárom érintett helyen (`plugins/relief_generator/source/registration.py`): az "Envelope" csoport 11, a "Torzítás" csoport 10, és a `sources` `item_schema`-beli `source_type` almezői közül 3 (összesen 24 mező) kapott `visible_when` értéket. Új tesztek igazolják a kezdeti rejtett állapotot, a vezérlő-váltásra történő átkapcsolást, a kétszintű lánc helyes kaszkádolását, és hogy a `values()` a rejtett mezők értékét is tartalmazza — plusz egy `_EXPECTED_PARAMETER_VISIBLE_WHEN` elvárás-szótár a `registration.py` retrofit teljes lefedettségének ellenőrzésére. Teljes tesztkészlet (646 teszt) regresszió nélkül zöld, `ruff`/`mypy --strict --explicit-package-bases` hibamentes.
...
> **Megjegyzés (2026-08-29, folytatás 57):** A 11.3 (Dűne-felszín) élő
...
> **Megjegyzés (2026-09-02, folytatás 64):** A 13.2 tétel (Image
> Interpretation) elkészült, review-n átment — a Szoftverarchitekt a
> jóváhagyott prompt "Cél" szakasza ellenében ellenőrizte mind az öt
> érintett fájlt, szó szerinti egyezéssel. A `docs/plugins/
> relief_generator/IMAGE_RELIEF_INTERPRETATION.md` Elfogadva
> státusszal a végleges helyén van; a kód-szintű mechanizmus
> (színkódolt régió-térkép + JSON hozzárendelés, `color_tolerance`
> alapú kvantálás, kötegelt hibajelentés) a `plugins/relief_generator/
> domain/` alá került, `ImageInterpretationError` kiegészítéssel és
> egy új plugin-szintű függőséggel (Pillow); a teljes tesztkészlet
> (835 teszt) regresszió nélkül zöld. A tétel tisztán domain-szintű,
> fájl-alapú mechanizmus (nincs GUI, ezt a 13.9 pótolja majd), ezért
> élő tesztelést nem igényelt. A Phase 13 aktív tétele mostantól a
> 13.3 (Region Resolution, ADR-0019).

> **Megjegyzés (2026-09-02, folytatás 65):** A 13.3 tétel (Region
> Resolution, ADR-0019) elkészült, review-n átment. A Szoftverarchitekt
> a jóváhagyott prompt "Cél" szakasza ellenében ellenőrizte mind az öt
> érintett fájlt; négy szó szerint egyezett, az ötödiknél (ADR-0019)
> egy hibát talált — a Szoftverarchitekt saját, korábbi promptjának
> belső ellentmondása miatt (a "Cél" teljes cserét írt elő, a
> "Korlátozások" tévesen a Kontextus szakasz megtartását sugallta)
> Claude Code a stub-állapotból két, immár elavult mondatot
> megtartott a Kontextus szakasz elején. Ezt egy külön, célzott
> korrekciós prompt javította — az ADR-0019 Kontextus szakasza most
> szó szerint egyezik a jóváhagyott tartalommal. A `docs/plugins/
> relief_generator/IMAGE_RELIEF_REGION_RESOLUTION.md` Elfogadva
> státusszal a végleges helyén van; a kód-szintű Resolver
> (`EffectSpec`, `resolve_regions`) a `plugins/relief_generator/
> domain/` alá került, `RegionResolutionError` kiegészítéssel; a
> teljes tesztkészlet (845 teszt) regresszió nélkül zöld. A tétel
...
> **Megjegyzés (2026-09-02, folytatás 67):** A 13.5 tétel (Relief
> Representation) elkészült, review-n átment — a Szoftverarchitekt a
> jóváhagyott prompt "Cél" szakasza ellenében ellenőrizte mind a
> három érintett fájlt (`IMAGE_RELIEF_RELIEF_REPRESENTATION.md`,
> `relief_representation.py`, `test_relief_representation.py`), szó
> szerinti egyezéssel. A teljes tesztkészlet (861 teszt) regresszió
> nélkül zöld.
>
> **Terminológiai önjavítás:** a "folytatás 66" jegyzet (fent,
> változatlan formában) tévesen nevezte a Semantic World réteget
> "13.1–13.4"-nek. A tervezési dokumentum
> (`docs/drafts/image_relief_generator/IMAGE_RELIEF_GENERATOR_PLANNING.md`)
> szerint helyesen: **Semantic World** = Image Interpretation → Region
> Resolver → EffectSpec[] (Phase 13.1–13.3); **Relief World** = Effect
> Processing → Relief Representation (Phase 13.4–13.5). A 13.4 (Effect
> Processing) tehát nem a Semantic World lezárása volt, hanem a Relief
> World *kezdete* — ami csak most, a 13.5-tel zárul le ténylegesen. A
> "folytatás 66" szövege történeti bejegyzésként változatlan marad;
> ezt a jegyzetet ez a bekezdés korrigálja, visszamenőleges szerkesztés
> nélkül.
>
> Ezzel mind a Semantic World (13.1–13.3), mind a Relief World
> (13.4–13.5) ténylegesen lezárult. A Phase 13 aktív tétele mostantól
> a 13.6 (`GeometricSurface`, ADR-0018).

> **Megjegyzés (2026-09-03, folytatás 68):** A 13.6 tétel
> (`GeometricSurface`) elkészült, review-n átment — a Szoftverarchitekt
> a jóváhagyott prompt "Cél" szakasza ellenében ellenőrizte mind az öt
> érintett fájlt (`ADR-0018`, `IMAGE_RELIEF_GEOMETRIC_SURFACE.md`,
> `geometric_surface.py`, `exceptions.py`, `test_geometric_surface.py`),
> szó szerinti egyezéssel; az `exceptions.py`-ban az új
> `GeometricSurfaceValueError` a fájl meglévő két-üres-sor
> konvencióját követve, a korábbi 23 osztály változatlanul hagyásával
> került be. A teljes tesztkészlet (869 teszt) regresszió nélkül zöld
> (861 → 869, pontosan a várt 8 új teszttel). A tétel tisztán
> domain-szintű, GUI-hoz még nem kötött, ezért élő tesztelést nem
> igényelt.
>
> A Phase 13 aktív tétele mostantól a 13.7 (Raw Mesh generálás) —
> ehhez a `docs/AI_WORKFLOW.md` "Kiegészítés procedurális Height
> Field receptekhez" pontja szerint matplotlib-előnézet kötelező
> jóváhagyás szükséges a Claude Code-nak szánt implementációs prompt
> előtt.

> **Megjegyzés (2026-09-03, folytatás 69):** A 13.7 tétel (Raw Mesh
> generálás) elkészült, review-n átment — a Szoftverarchitekt a
> jóváhagyott prompt "Cél" szakasza ellenében ellenőrizte mind a hét
> érintett fájlt (`ADR-0020`, `IMAGE_RELIEF_RAW_MESH.md`, a két
> célzott kiegészítés `IMAGE_RELIEF_GEOMETRIC_SURFACE.md`-ben és a
> tervezési draftban, `geometric_surface_mesh_generator.py`,
> `exceptions.py`, `test_geometric_surface_mesh_generator.py`). A
> kód/teszt fájloknál három helyen tisztán kozmetikai sortördelési
> eltérés (a `ruff format` 88 karakteres sorhossz-szabálya miatt) —
> tartalmi eltérés nélkül, a Szoftverarchitekt ezt jogos, indokolt
> korrekcióként fogadta el, nem hibaként. A teljes tesztkészlet (879
> teszt) regresszió nélkül zöld (869 → 879, pontosan a várt 10 új
> teszttel). Ez az alfázis a `docs/AI_WORKFLOW.md` szerinti
> matplotlib-előnézeti jóváhagyáson is átment (szintetikus
> House/Window/Door példa) — az előnézet és a hozzá tartozó
> példa-fájlok (`example_house.png`, `example_house_assignment.json`)
> a projektgazdának átadva, későbbi tesztekhez felhasználhatók.
>
